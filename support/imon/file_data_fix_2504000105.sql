...
BEGIN TRANSACTION 
begin
DECLARE     @AGREEMENT_NO VARCHAR(20) = '0001159/4/08/10/2023'
...
EXEC dbo.xsp_mtn_hold_maturity_v2 @p_agreement_no = @AGREEMENT_NO, -- nvarchar(50)
                                  @p_mod_by = N'MTN-FAUZAN'        -- nvarchar(50)
SELECT		'HANDOVER_ASSET',STATUS,* FROM IFINAMS.dbo.HANDOVER_ASSET WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
SELECT		'HANDOVER_REQUEST',STATUS,* FROM IFINAMS.dbo.HANDOVER_REQUEST WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
select		'MATURITY'
			,ma.code
...
BEGIN TRANSACTION 
begin
DECLARE     @AGREEMENT_NO VARCHAR(20) = '0001160/4/08/10/2023'
SELECT		'HANDOVER_ASSET',STATUS,* FROM IFINAMS.dbo.HANDOVER_ASSET WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
SELECT		'HANDOVER_REQUEST',STATUS,* FROM IFINAMS.dbo.HANDOVER_REQUEST WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
select		'MATURITY'
			,ma.code
...
BEGIN TRANSACTION 
begin
DECLARE     @AGREEMENT_NO VARCHAR(20) = '0000957/4/01/08/2022'
SELECT		'HANDOVER_ASSET',STATUS,* FROM IFINAMS.dbo.HANDOVER_ASSET WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
SELECT		'HANDOVER_REQUEST',STATUS,* FROM IFINAMS.dbo.HANDOVER_REQUEST WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
select		'MATURITY'
			,ma.code
