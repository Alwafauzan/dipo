ALTER PROCEDURE dbo.xsp_asset_update_rental_status
(
	@p_code			   nvarchar(50)
...
begin
	...
	begin try
		...
		begin
			...
			--(+) fauzan
			--note TO kak sep : tapi bukannya kalo udah UPDATE di bawah bakal otomatis keganti ya kak semua yg code nya sama?
			IF EXISTS ( SELECT 1 FROM dbo.ASSET WHERE code = @p_code and rental_reff_no = @p_rental_reff_no )
			BEGIN
			update	asset
