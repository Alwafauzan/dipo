ALTER PROCEDURE dbo.xsp_application_asset_update
(
...
begin
	...
	begin try 
		...
		if (@is_simulation <> '1') AND (len(@p_client_nitku) <> 6)
		begin 
		...
		begin
			set @sum_asset_year = year(getdate()) - @asset_year ;
		...
		--(+)fauzan
		exec ifinams.dbo.xsp_asset_update_rental_status @p_code				= @p_fa_code
														,@p_rental_reff_no	= @p_asset_no
