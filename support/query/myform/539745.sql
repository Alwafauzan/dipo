--539745
BEGIN TRANSACTION 
EXEC dbo.xsp_mtn_hold_maturity_v2 @p_agreement_no = N'0002896/4/08/09/2024', -- nvarchar(50)
                                  @p_mod_by = N''        -- nvarchar(50)
declare     @AGREEMENT_NO VARCHAR(20) = '0002896/4/08/09/2024'
...
--declare     @AGREEMENT_NO VARCHAR(20) = '0001141/4/08/09/2023'
SELECT		'HANDOVER_ASSET',STATUS,* FROM IFINAMS.dbo.HANDOVER_ASSET WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
SELECT		'HANDOVER_REQUEST',STATUS,* FROM IFINAMS.dbo.HANDOVER_REQUEST WHERE REMARK like '%'+replace(@AGREEMENT_NO,'.','/')+'%'
select		'MATURITY'
			,ma.code
